...
def load_mapping_data(ckpt, dataset):
    ...
    # Load codebook-based id2meta
    with open(f"./data/{dataset}_id2meta_codebook.json", 'r', encoding='utf-8') as f:
...
def create_sft_data(parent_asin2meta, user_interactions, ckpt, dataset, min_items=6, words_per_item=None):
    ...
    """
    words_per_item = TOKENS_PER_ITEM  # Always 3 for codebook
...
def create_instruction_sample(input_words, output_words, user_id, total_items, item_id_list, meta_msg_list, words_per_item=5):
    ...
    """
    # Instruction text
    instruction = f"Given a user's historical item interaction sequence, predict the keywords of the next item the user is most likely to interact with. Each item in the sequence is represented by a unique identifier composed of {words_per_item} special tokens enclosed in square brackets []. The items are listed in chronological order.\n"
    
    # Input text - first item
    input_text = f"Item keywords: {format_sid(input_words)}"
    ...
    # Output text - remaining items
    output_text = ""
    for i in range(total_items-3):
        output_text += f"Item keywords: {format_sid(meta_msg_list[i+1]['summary_words'])}"
...
def main(ckpt, dataset):
    ...
    # Save data
    output_file = f"./train_data/{dataset}_sid_next_item_prediction_sft.json"
