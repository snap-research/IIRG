...
def main():
    parser = argparse.ArgumentParser(
        description="Merge SID (codebook) SFT dataset files with per-source loss weights."
    )
    ...
    input_files = [
        (input_dir / f"{args.dataset}_sid_collaborative_neighbors_sft.json", args.copurchase_weight),
        (input_dir / f"{args.dataset}_sid_semantic_neighbors_sft.json", args.semantic_weight),
        (input_dir / f"{args.dataset}_sid_next_item_prediction_sft_simplified.json", args.rec_weight),
    ]

    out_path = output_dir / f"{args.dataset}_sid_merged_sft.jsonl"
