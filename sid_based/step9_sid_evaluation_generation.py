...
def prepare_batch_prompts(batch_data):
    """Prepare prompts for batch data — aligned with step4_prediction_dataset_codebook.py"""
    batch_prompts = []
    batch_metadata = []
    ...
    for d in batch_data:
        ...
        l = d["input"] + d["output"]
        l += f"Item keywords: {format_sid(d['valid_ground_truth_tid'])}"
        ...
        instruction = (
            "Given a user's historical item interaction sequence, predict the next item "
            "the user is most likely to interact with. Each item in the sequence is "
            f"represented by a unique identifier composed of {TOKENS_PER_ITEM} special tokens "
            "enclosed in square brackets []. The items are listed in chronological order.\n"
        )
...
def process_single_gpu(rank, data_slice, output_queue, model_name, batch_size=8):
...
def main(model_path, dataset_name):
    ...
    for ckpt in ckpts:
        ...
        test_file = f"./train_data/{dataset_name}_sid_next_item_prediction_sft.json"
        ...
        output_file = f"./results/{dataset_name}_generation_outputs_sid.json"
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Generation script for codebook recommendation model')
    parser.add_argument('--dataset_name', type=str, default='toys',
                        help='Name of the dataset to evaluate on')
    parser.add_argument('--model_path', type=str, default=None,
                        help='Path to the model checkpoint (default: model_{dataset_name}_sid_sft)')
    args = parser.parse_args()

    if args.model_path is None:
        args.model_path = f"model_{args.dataset_name}_sid_sft"
